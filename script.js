		...
		return this.each(function(){
			var th=$(this)
				,_=th.data('forms')||{
					...
					successCl:'success',
					successShow:'4000',					
					mailHandlerURL: templateUrl+"/mail/MailHandler.php ",
					ownerEmail:'',
					stripHTML:true,
...
$(window).bind("load", function() {     
    $('.scrollbar1').tinyscrollbar({ sizethumb: 134 });
});
...
$(window).load(function(){
    ...
    function resizeContent(_animationSpeed)
    {
        ...
        if (window_H > mainDIV_H) {
			leftSide.stop().animate({paddingTop:~~((window_H - mainDIV_H)/2)}, _animationSpeed, 'easeOutCubic');	
			content.stop().animate({paddingTop:~~((window_H - mainDIV_H)/2)}, _animationSpeed, 'easeOutCubic');	
		} else {
			leftSide.stop().animate({paddingTop:0}, _animationSpeed, 'easeOutCubic');	
			content.stop().animate({paddingTop:0}, _animationSpeed, 'easeOutCubic');	
		}
		slider.stop().animate({top:~~((window_H - slider.height())/2)}, _animationSpeed, 'easeOutCubic');	
    }
    
    resizeContent(0);
    
    $('.icon1, .icon2, .icon3, .icon4').sprites({
		method:'simple',
		duration:400,
		easing:'easeOutQuint',
		hover:true
    ...
    $('.scroll, .scroll2, .scroll3, .scroll4').cScroll({
        duration:700,
		step:100,
		trackCl:'track',
		shuttleCl:'shuttle',
        hoverIn: function(_shuttle){
            _shuttle.stop().css({'backgroundPosition':'right 0'});
        },
        hoverOut: function (_shuttle){
            _shuttle.stop().css({'backgroundPosition':'left 0'});
        }
    })
    
    
    /*
		Gallery Column Hover effect
	*/
	var galleryColumnBoxes = $('.gallery .columnBox');
	...
	/* end Gallery Column Hover effect */
	
	 /*
		Gallery Column Hover effect for Single Portf
	*/
	var galleryColumnBoxes = $('.singlePortfImage');
	galleryColumnBoxes.live({
	    mouseenter: function(){ // hover in
            var img = $(this).find('a').children('img');
            var overlay = $(this).find('.overlay');
	...
	/* 	Gallery Column Hover effect for Single Portf*/
	 ...
	 /*
		Gallery Column Hover effect for Blog
	*/
	var galleryColumnBoxes = $('.galleryBlog .blogTemplateImg');
	galleryColumnBoxes.live({
	    mouseenter: function(){ // hover in
            var img = $(this).find('a').children('img');
            var overlay = $(this).find('.overlay');
	...
	/* end Gallery Column Hover effect for Blog */
    ...
    nav.navs({
        ...
        defHash:'#!/',
		hoverIn:function(li){
		   	$('>a',li).stop().animate(250,'easeOutSine');
		},
     ...
     content.tabs({
		preFu:function(_)
        {
            _.li.css({display:'none', left:'-720px'});
		}
		,actFu:function(_)
        {
            ...
            if(_.n == -1){
                slider.stop().delay(aniDelay).animate({left:'220px'}, 650, 'easeOutCubic');
            } else {
                 slider.stop().animate({left:'-510px'}, 300, 'easeInSine');
        	...
        	if(_.curr){
        	   _.curr 
        	       .css({display:'block'})
                   .stop()
                        .delay(aniDelay).animate({left:'0px'}, 650, 'easeOutCubic');
			...
			if(_.prev){
			   _.prev 
        	       .stop()
                        .animate({left:'-720px'}, 300, 'easeInSine', function(){
  ...
  // attempt to call Quicksand when a filter option
	...
	$('.filterOptions li a').click(function(e) {
		...
		$holder.quicksand($filteredData, {
			...
			$('.imgList3 li a.portfPic').fancybox({
					'overlayColor'  :   '#1c1c1c',
					'transitionIn'	:	'elastic',
					'transitionOut'	:	'elastic',
					'speedIn'		:	500, 
...
$(document).ready(function() {
  ...
  // get the action filter option item on page load
  var $filterType1 = $('.filterOptions1 li.active a').attr('class');
	
  // get and assign the ourHolder element to the
	// $holder varible for use later
  var $holder1 = $('ul.ourHolder1');
  ...
  // attempt to call Quicksand when a filter option
	// item is clicked
	$('.filterOptions1 li a').click(function(e) {
		// reset the active class on all the buttons
		$('.filterOptions1 li').removeClass('activeF');
		...
		if ($filterType1 == 'all') {
			// assign all li items to the $filteredData var when
			// the 'All' filter option is clicked
			var $filteredData1 = $data1.find('li');
		} 
		else {
			// find all li elements that have our required $filterType
			// values for the data-type element
			var $filteredData1 = $data1.find('li[data-type~=' + $filterType1 + ']');
		}
...
$(document).ready(function() {
  ...
  // get the action filter option item on page load
  var $filterType2 = $('.filterOptions2 li.active a').attr('class');
	
  // get and assign the ourHolder element to the
	// $holder varible for use later
  var $holder2 = $('ul.ourHolder2');
  ...
  // attempt to call Quicksand when a filter option
	// item is clicked
	$('.filterOptions2 li a').click(function(e) {
		// reset the active class on all the buttons
		$('.filterOptions2 li').removeClass('activeF');
		...
		if ($filterType2 == 'all') {
			// assign all li items to the $filteredData var when
			// the 'All' filter option is clicked
			var $filteredData2 = $data2.find('li');
		} 
		else {
			// find all li elements that have our required $filterType
			// values for the data-type element
			var $filteredData2 = $data2.find('li[data-type~=' + $filterType2 + ']');
		}
...
$(document).ready(function() {
  ...
  // get the action filter option item on page load
  var $filterType4 = $('.filterOptions4 li.active a').attr('class');
	
  // get and assign the ourHolder element to the
	// $holder varible for use later
  var $holder4 = $('ul.ourHolder4');

  // clone all items within the pre-assigned $holder element
  var $data4 = $holder4.clone();

  // attempt to call Quicksand when a filter option
	// item is clicked
	$('.filterOptions4 li a').click(function(e) {
		// reset the active class on all the buttons
		$('.filterOptions4 li').removeClass('activeF');
		
		// assign the class of the clicked filter option
		// element to our $filterType variable
		var $filterType4 = $(this).attr('class');
		$(this).parent().addClass('activeF');
		
		if ($filterType4 == 'all') {
			// assign all li items to the $filteredData var when
			// the 'All' filter option is clicked
			var $filteredData4 = $data4.find('li');
		} 
		else {
			// find all li elements that have our required $filterType
			// values for the data-type element
			var $filteredData4 = $data4.find('li[data-type~=' + $filterType4 + ']');
		}
		
		// call quicksand and assign transition parameters
		$holder4.quicksand($filteredData4, {
			duration: 800,
			easing: 'easeInOutQuad',
			}, function() {
